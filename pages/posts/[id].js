...
import postsPageInitialPaths2856eResource from '../../resources/posts-page-initial-paths-2856e'
import postsPageInitialProps51387Resource from '../../resources/posts-page-initial-props-51387'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Corporate Brand Associate</title>
          <meta
            property="og:title"
            content="Posts1 - Corporate Brand Associate"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths2856eResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps51387Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
